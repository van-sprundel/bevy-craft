...
fn add_assets(
    ...
    let box_mesh_handle = meshes.add(Mesh::from(shape::Cube { size: 1. }));
        ...
        for y in 0..NUM_CUBES {
            for z in 0..NUM_CUBES {
                ...
                let task = thread_pool.spawn(async move {
                    ...
                    let duration = Duration::from_secs_f32(rng.gen_range(0.05..0.1));
...
fn handle_tasks(
    ...
    for (entity, mut task) in transform_tasks.iter_mut() {
        if let Some(mesh) = future::block_on(future::poll_once(&mut *task)) {
            ...
            commands.entity(entity).remove::<Task<Mesh>>();
        }
    }
}
